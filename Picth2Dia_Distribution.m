...
for i= 1 :Bsize
    ...
    for ii = 1:Msize
        ...
        for Ri = 1: size(P2D_box,1) %row
            for Ci = 1 : size(P2D_box,2) %column
                
                if (Ri == 10)&&(Ci == 10)
                    P2D_box(Ri,Ci,3) = max(max(P2D_box(:,:,3)));
                    continue 
                end
                
                Pdia = P2D_box(Ri,Ci,1); %Propeller's diameter
                Ppicth = P2D_box(Ri,Ci,2); %Propeller's picth
              ...
              elseif Pmax > Mpowermax %check that can motor use with propeller
               ...
               %    Endurance = 60/1000*Cap*v/(N_m*EPower_hover);
                ...
                P2D_box(Ri,Ci,3) = round(EPower_hover);   
        ...
        figure(1)
        clf %clear figure
        Max = max(max(P2D_box(:,:,3)))+1; %+1 for round down error
        Min = min(min(P2D_box(:,:,3)));
        cm = colormap(parula(Max-Min)); %make color map from min & max P
        hold on
        for ri = 1:size(P2D_box,1)
            for ci = 1:size(P2D_box,2)
                show(ri,ci) = stem3(ri,ci,P2D_box(ri,ci,3)); % plot 
                cm_idx = max( P2D_box(ri,ci,3)-Min, 1 );
        ...
        title(['Battery : ' Batt.textdata{i} ' Motor : ' Motor.textdata{ii} ' -Picth2Dia Dis'])
