%Power calculator
function [EPower,AUW,c] = Power_Calculation(Pdia,Ppicth,Wp,w_batt,w_motor,S_factor,Vc,N_m,Mi0,MR,M_maxrpm,Mpowermax,kv,v,b,hover_time,cruise_time,V0,Bcruise)
...
if err ~= 1
    ...
    if err == 2 %Set high value
        ...
        x = RPMhover;
        eff_h = (Eff_motor(MR,Mi0,kv,v,x));%find eletrict power for hover
        EPower_hover = Phover/eff_h;%(Eff_motor(MR,Mi0,kv,v,x));%find eletrict power for hover
        x = RPMcruise;
        eff_c = (Eff_motor(MR,Mi0,kv,v,x));
        EPower_cruise = Pcruise/eff_c;%(Eff_motor(MR,Mi0,kv,v,x));%find eletrict power for cruise
